...
{	
	...
	if (key_press.status_old != key_press.status_new) //if statues of key had chenged
	{
		key_press.change_time_ms_new = sys_time_now_ms;
		if (key_press.change_time_ms_new - key_press.change_time_ms_old >= KEY_VALID_CHANGE_MS) //if the change of key statues is valide
		{
